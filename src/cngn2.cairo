...
pub mod Cngn2 {
    ...
    pub enum Event {
        ...
        UpdateAdminOperations: UpdateAdminOperations,
    ...
    #[derive(Drop, starknet::Event)]
    pub struct UpdateAdminOperations {
        #[key]
    ...
    #[derive(Drop, starknet::Event)]
    pub struct UpdateForwarderContract {
        #[key]
        pub old_address: ContractAddress,
        #[key]
        pub new_address: ContractAddress,
    ...
    #[abi(embed_v0)]
    impl Cngn2Impl of super::ICngn2<ContractState> {
        ...
        fn update_admin_operations_address(
            ...
            assert(!new_admin.is_zero(), 'Admin address cannot be zero');

            let old_address = self.admin_operations_contract.read();
            self.admin_operations_contract.write(new_admin);

            self.emit(UpdateAdminOperations { old_address: old_address, new_address: new_admin });
        ...
        fn update_forwarder_contract(
            ...
            assert(!new_forwarder_contract.is_zero(), 'Forwarder cannot be zero');

            let old_address = self.trusted_forwarder_contract.read();
            self.trusted_forwarder_contract.write(new_forwarder_contract);

            self
                .emit(
                    UpdateForwarderContract {
                        old_address: old_address, new_address: new_forwarder_contract,
                    },
                );
        ...
        fn destroy_black_funds(ref self: ContractState, blacklisted_user: ContractAddress) -> bool {
            ...
            assert(admin_contract.is_black_listed(blacklisted_user), 'Not blacklisted');
...
#[starknet::interface]
pub trait ICngn2<TContractState> {
